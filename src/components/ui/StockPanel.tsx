'use client'
// src/components/ui/StockPanel.tsx
...
interface Mouvement {
  ...
  versionApres: number
  createdAt: string
  stock: { produit: { nom: string; unite: string } }
}

const TYPE_MOUVEMENT_STYLE: Record<string, { label: string; color: string; icon: string }> = {
  ...
  const handleAppro = async (produitId: string) => {
    ...
    try {
      const res = await fetch('/api/stock', {
        ...
        body: JSON.stringify({ produitId, quantite: parseFloat(approQte), description: approDesc || 'Approvisionnement' }),
      })
      if (res.ok) {
        toast('Stock approvisionné !')
        setShowAppro(null); setApproQte(''); setApproDesc('')
        fetchData()
      } else {
        const d = await res.json(); toast(d.error || 'Erreur', 'error')
      }
      ...
      {totalAlerte > 0 && (
        <div className="flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl">
          ...
          <p className="text-sm text-amber-800 font-medium">
            {totalAlerte} produit{totalAlerte > 1 ? 's' : ''} en dessous du seuil d&apos;alerte
          </p>
        </div>
      )}

      {/* ── Légende du verrouillage optimiste ── */}
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm text-blue-800">
        <p className="font-semibold mb-1">🔐 Mécanisme anti-conflit actif (verrouillage optimiste)</p>
        <p className="text-blue-700 text-xs leading-relaxed">
          Si deux utilisateurs commandent simultanément le même produit, le système garantit que la somme des quantités accordées ne dépasse jamais le stock disponible.
          Chaque écriture est conditionnelle à une <strong>version</strong> — si elle a changé entre lecture et écriture, la requête est retenté automatiquement (max 3 fois).
        </p>
      </div>
        ...
        {stocks.map(s => {
          ...
          return (
            <div key={s.id} className={cn(
              ...
              s.enAlerte ? 'border-amber-300' : 'border-slate-200'
            )}>
              <div className="flex items-start justify-between mb-3">
                ...
                <button
                  onClick={() => { setShowAppro(s.produitId); setApproQte(''); setApproDesc('') }}
              ...
              <div className="mb-3">
                ...
                <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    className={cn('h-full rounded-full transition-all', s.enAlerte ? 'bg-amber-400' : 'bg-green-500')}
              ...
              <div className="grid grid-cols-3 gap-2 text-center text-xs">
                <div className="bg-slate-50 rounded-lg p-1.5">
                  <p className="font-bold text-slate-800">{s.quantiteDisponible}</p>
                  <p className="text-slate-400">Total</p>
                </div>
                <div className="bg-blue-50 rounded-lg p-1.5">
                  <p className="font-bold text-blue-700">{s.quantiteReservee}</p>
                  <p className="text-blue-500">Réservé</p>
                </div>
                <div className={cn('rounded-lg p-1.5', s.enAlerte ? 'bg-amber-50' : 'bg-green-50')}>
                  <p className={cn('font-bold', s.enAlerte ? 'text-amber-700' : 'text-green-700')}>{s.quantiteLibre}</p>
                  <p className={s.enAlerte ? 'text-amber-500' : 'text-green-500'}>Libre</p>
                </div>
              </div>

              {/* Seuil alerte */}
              <p className="text-xs text-slate-400 mt-2">
                Seuil alerte : {s.seuilAlerte} {s.unite}
              </p>

              {/* Formulaire appro inline */}
              {showAppro === s.produitId && (
                <div className="mt-3 pt-3 border-t border-slate-100 space-y-2">
                  ...
                  <input
                    ...
                    step="0.1"
                    value={approQte}
                    onChange={e => setApproQte(e.target.value)}
                    placeholder={`Quantité (${s.unite})`}
                  ...
                  <input
                    type="text"
                    value={approDesc}
                    onChange={e => setApproDesc(e.target.value)}
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => handleAppro(s.produitId)} disabled={submitting}
                      className="flex-1 text-xs py-1.5 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50">
                      Confirmer
                    </button>
                  </div>
                </div>
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 sticky top-0">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-left text-xs font-semibold text-slate-500 hidden sm:table-cell">Description</th>
                    <th className="px-4 py-2 text-right text-xs font-semibold text-slate-500">Date</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {mouvements.map(m => {
                    const style = TYPE_MOUVEMENT_STYLE[m.type] || { label: m.type, color: 'bg-slate-100 text-slate-700', icon: '•' }
                    return (
                      <tr key={m.id} className="hover:bg-slate-50">
                        <td className="px-4 py-2.5">
                          <span className={cn('badge', style.color)}>{style.icon} {style.label}</span>
                        </td>
                        ...
                        <td className="px-4 py-2.5 text-right font-mono text-xs text-slate-700">
                          {['RESERVATION', 'LIVRAISON'].includes(m.type) ? '-' : '+'}
                          {m.quantite} {m.stock.produit.unite}
                        </td>
                        <td className="px-4 py-2.5 text-center">
                          <span className="text-xs font-mono text-slate-400">v{m.versionApres}</span>
                        </td>
                        <td className="px-4 py-2.5 text-xs text-slate-500 hidden sm:table-cell truncate max-w-xs">{m.description || '—'}</td>
                        <td className="px-4 py-2.5 text-xs text-slate-400 text-right whitespace-nowrap">
