...
import { formatDate, STATUT_COMMANDE_LABELS } from '@/lib/utils'
import { infoFenetreCommande } from '@/lib/dijkstra'
import { cn } from '@/lib/utils'
import { toast } from '@/components/ui/Toaster'

interface Point { id: string; nom: string; adresse: string }
interface Produit { id: string; nom: string; unite: string; description: string | null }
interface StockInfo { produitId: string; nomProduit: string; unite: string; quantiteLibre: number; enAlerte: boolean }
interface Commande {
  id: string; reference: string; quantite: number; statut: string
  heurePassee: string; notes: string | null
  point: Point; produit: Produit
}
...
export default function CommandesPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setCommandes(cmd)
      setPoints(pts.filter((p: any) => p.actif))
      setProduits(prods.filter((p: any) => p.actif))
      setStocks(stks)
  ...
  const handleSubmit = async (forceHoraire = false) => {
    if (!form.pointId || !form.produitId || !form.quantite) {
      toast('Veuillez remplir tous les champs obligatoires', 'error'); return
    }
    if (stockInsuffisant) {
      toast(`Stock insuffisant — disponible : ${stockProduit!.quantiteLibre} ${stockProduit!.unite}`, 'error'); return
    }
    setSubmitting(true)
    setErreurStock(null)
    try {
      ...
      if (!res.ok) {
        if (data.code === 'HORS_FENETRE') {
          if (confirm('Hors fenêtre horaire. Forcer (admin) ?')) {
            setSubmitting(false)
            return handleSubmit(true)
          }
        } else if (data.code === 'STOCK_INSUFFISANT') {
          setErreurStock(data.error)
        } else if (data.code === 'CONFLIT_EPUISE') {
          toast('⚡ Conflit de concurrence — le stock vient d\'être attribué. Réessayez.', 'error')
        } else {
      ...
      setShowForm(false)
      setForm({ pointId: '', produitId: '', quantite: '', notes: '' })
      setErreurStock(null)
      fetchData()
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <ShoppingCart className="h-6 w-6 text-primary-600" />Gestion des commandes
          </h1>
          <p className="text-slate-500 text-sm mt-1">Fenêtre de commande : 20h00 – 23h00 · Anti-conflits actif</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setShowForm(true)} className="flex items-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-lg hover:bg-primary-700">
      ...
      <div className={cn('flex items-center gap-3 p-4 rounded-xl border mb-6', fenetre.ouverte ? 'bg-green-50 border-green-200' : 'bg-amber-50 border-amber-200')}>
        ...
        <div>
          <p className={cn('font-semibold', fenetre.ouverte ? 'text-green-800' : 'text-amber-800')}>
            {fenetre.ouverte ? '🟢 Fenêtre ouverte' : '🔴 Fenêtre fermée'}
          </p>
        ...
        {['TOUS', 'EN_ATTENTE', 'VALIDEE', 'EN_COURS_LIVRAISON', 'LIVREE', 'ANNULEE'].map(s => (
          <button key={s} onClick={() => setFiltre(s)}
            className={cn('px-3 py-1.5 rounded-lg text-xs font-medium transition-colors',
              filtre === s ? 'bg-primary-600 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50')}>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-slide-up">
            <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
              <Package className="h-5 w-5 text-primary-600" />Nouvelle commande
            </h2>

            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Point de distribution *</label>
                <select value={form.pointId} onChange={e => setForm(f => ({ ...f, pointId: e.target.value }))}
              ...
              <div>
                ...
                <select value={form.produitId} onChange={e => setForm(f => ({ ...f, produitId: e.target.value, quantite: '' }))}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                  ...
                  {produits.map(p => {
                    ...
                    return (
                      <option key={p.id} value={p.id}>
                        {p.nom} {s ? `— ${s.quantiteLibre} ${p.unite} libres` : ''}
                      </option>
                    )
                  })}
                </select>

                {/* Stock du produit choisi */}
                {stockProduit && (
                  <div className={cn('mt-2 flex items-center gap-2 px-3 py-2 rounded-lg text-xs',
                    stockProduit.enAlerte ? 'bg-amber-50 border border-amber-200 text-amber-700' : 'bg-green-50 border border-green-200 text-green-700')}>
                    {stockProduit.enAlerte
                      ? <AlertTriangle className="h-3.5 w-3.5 flex-shrink-0" />
                      : <Package className="h-3.5 w-3.5 flex-shrink-0" />}
                    <span>
                      Stock disponible : <strong>{stockProduit.quantiteLibre} {stockProduit.unite}</strong>
                      {stockProduit.enAlerte && ' ⚠️ Alerte stock bas'}
                    </span>
                  </div>
                )}
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                  Quantité ({stockProduit?.unite || 'unité'}) *
                </label>
                <input type="number" min="0.1" step="0.1" value={form.quantite}
                  onChange={e => { setForm(f => ({ ...f, quantite: e.target.value })); setErreurStock(null) }}
                  ...
                  className={cn('w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2',
                    stockInsuffisant ? 'border-red-400 focus:ring-red-500' : 'border-slate-300 focus:ring-primary-500')} />

                {/* Alerte stock insuffisant en temps réel */}
                {stockInsuffisant && (
                  <p className="text-xs text-red-600 mt-1 flex items-center gap-1">
                    <AlertTriangle className="h-3 w-3" />
                    Stock insuffisant — max disponible : {stockProduit!.quantiteLibre} {stockProduit!.unite}
                  </p>
                )}
              </div>
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => { setShowForm(false); setErreurStock(null) }}
                className="flex-1 px-4 py-2 border border-slate-300 rounded-lg text-sm text-slate-700 hover:bg-slate-50">Annuler</button>
              <button onClick={() => handleSubmit(false)} disabled={submitting || !!stockInsuffisant}
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    {cmd.statut === 'EN_ATTENTE' && (
                      <button onClick={() => updateStatut(cmd.id, 'VALIDEE')} title="Valider"
                        className="p-2 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100">
                        <CheckCircle className="h-4 w-4" />
                      </button>
                    )}
                    {['EN_ATTENTE', 'VALIDEE'].includes(cmd.statut) && (
                      <button onClick={() => updateStatut(cmd.id, 'ANNULEE')} title="Annuler"
