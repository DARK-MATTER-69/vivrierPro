...
import { formatDate, formatDistance, formatDuree, STATUT_LIVRAISON_LABELS, VEHICULE_CONFIG } from '@/lib/utils'
...
interface Livraison {
  id: string; reference: string; statut: string; distanceTotale: number
  tempsPrevuMin: number; dateDepart: string; dateArrivee: string | null
  vehicule: Vehicule
...
export default function LivraisonsPage() {
  ...
  // Prévisualiser l'itinéraire optimal quand les commandes changent
  const previewRoute = useCallback(async () => {
    if (selectedCommandes.length === 0) { setSelectedPath([]); return }
    const pointsIds = [...new Set(
      commandesValidees.filter(c => selectedCommandes.includes(c.id)).map(c => c.point.id)
    )]
    // Calculer via l'API graphe
  ...
  const handlePlanifier = async () => {
    if (!selectedVehicule || selectedCommandes.length === 0) {
      toast('Sélectionnez un véhicule et au moins une commande', 'error'); return
    }
    const vehicule = vehicules.find(v => v.id === selectedVehicule)
    if (selectedCommandes.length > (vehicule?.capacite ?? 0)) {
      toast(`Ce véhicule peut couvrir max ${vehicule?.capacite} point(s) par tournée`, 'error'); return
    }
    setCreating(true)
    try {
      const res = await fetch('/api/livraisons', {
        ...
        body: JSON.stringify({ vehiculeId: selectedVehicule, commandeIds: selectedCommandes }),
      })
      ...
      setSelectedVehicule(''); setSelectedCommandes([])
      fetchAll()
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Planifiez les tournées avec optimisation de l&apos;itinéraire (Dijkstra)</p>
        </div>
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={commandesValidees.length === 0}
            className="flex items-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-lg hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      {commandesValidees.length === 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 text-sm text-amber-800">
          ⚠️ Aucune commande validée. Validez d&apos;abord des commandes dans l&apos;onglet <strong>Commandes</strong>.
        </div>
      )}
                ...
                <div key={type} className="flex items-center justify-between p-2 bg-slate-50 rounded-lg">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{cfg.vitesse} km/h · {cfg.capacite} pt/tournée</p>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-6 animate-slide-up max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-5">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-lg">{cfg?.icon}</span>
                        <span className="font-medium text-sm text-slate-800">{v.immatriculation}</span>
                      </div>
              ...
              {vehicules.filter(v => v.disponible).length === 0 && (
                <p className="text-sm text-red-500 mt-2">Aucun véhicule disponible</p>
              )}
            </div>
              ...
              <label className="block text-sm font-semibold text-slate-700 mb-2">
                ...
                {config && (
                  <span className="font-normal text-slate-500 ml-2">
                    (max {config.capacite} points · {selectedCommandes.length} sélectionné{selectedCommandes.length > 1 ? 's' : ''})
                  </span>
                )}
              </label>
              <div className="space-y-2 max-h-48 overflow-y-auto border border-slate-200 rounded-xl p-2">
                {commandesValidees.map(cmd => (
                  <label key={cmd.id} className={cn(
                    'flex items-center gap-3 p-2.5 rounded-lg cursor-pointer transition-colors',
                    selectedCommandes.includes(cmd.id) ? 'bg-primary-50' : 'hover:bg-slate-50'
                  )}>
                    <input
                      type="checkbox"
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-slate-800">{cmd.point.nom}</p>
                      <p className="text-xs text-slate-500">{cmd.reference} · {cmd.quantite} kg</p>
                    </div>
            ...
            {selectedVehicule && selectedCommandes.length > 0 && config && (
              <div className="bg-green-50 border border-green-200 rounded-xl p-3 mb-5 text-sm">
                ...
                <p className="text-green-700">
                  {config.icon} {vehiculeChoisi?.immatriculation} · {selectedCommandes.length} commande(s) · Vitesse {config.vitesse} km/h
                </p>
                <p className="text-green-600 text-xs mt-1">
                  L&apos;itinéraire optimal sera calculé via l&apos;algorithme de Dijkstra
                </p>
              </div>
            )}

            <div className="flex gap-3">
              <button onClick={() => { setShowPlanifier(false); setSelectedCommandes([]); setSelectedVehicule('') }}
                className="flex-1 px-4 py-2 border border-slate-300 rounded-lg text-sm text-slate-700 hover:bg-slate-50">
                Annuler
              </button>
              <button onClick={handlePlanifier} disabled={creating || !selectedVehicule || selectedCommandes.length === 0}
            ...
            return (
              <div key={liv.id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 card-hover">
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-mono text-xs text-slate-400">{liv.reference}</span>
                  ...
                  <div className="flex gap-2">
                    ...
                    {liv.statut === 'EN_COURS' && (
                      <button onClick={() => updateStatut(liv.id, 'TERMINEE')}
                        className="px-3 py-1.5 bg-green-100 text-green-700 rounded-lg text-xs font-medium hover:bg-green-200">
                        <CheckCircle className="h-3.5 w-3.5 inline mr-1" />Terminer
                      </button>
                    )}
                    {['PLANIFIEE', 'EN_COURS'].includes(liv.statut) && (
                      <button onClick={() => updateStatut(liv.id, 'ANNULEE')}
